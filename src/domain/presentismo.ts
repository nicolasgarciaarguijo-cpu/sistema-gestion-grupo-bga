...
//
// El presentismo es el 10% del NETO, y se cobra entero solo si el empleado cumplio con la
// presencialidad Y con el horario de llegada y salida. Cada falta lo va comiendo:
//
...
export const PRESENTISMO_PCT_DEL_NETO = 10;
...
export type Presentismo = {
  // Lo que le corresponderia con asistencia perfecta (10% del neto).
  base: number;
...
export function calcularPresentismo(input: {
  ...
  const base = Math.round(((Number(input.netoBase) || 0) * PRESENTISMO_PCT_DEL_NETO) / 100 * 100) / 100;
